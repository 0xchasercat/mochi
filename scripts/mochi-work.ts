...
export async function isBranchMerged(branch: string, repo: RepoCtx): Promise<boolean> {
  // CRITICAL: branch tip == origin/main tip means the branch is fresh from
  // `bun work create` and the agent hasn't committed yet. Treat as in-flight,
  // do NOT reap. (Only this exact-tip-match case; a branch *behind* main with
  // zero ahead-commits is genuinely stale and DOES merge-classify as ancestor.)
  // Users wanting to remove an in-flight no-commit worktree anyway use `--all`.
  // Refs: tasks/0012-bun-work-squash-detection.md (this supersedes the
  //   "new branch with zero unique commits → merged" rule from that brief).
  const branchTipResult = await run(["git", "rev-parse", branch], { cwd: repo.root });
  const mainTipResult = await run(["git", "rev-parse", "origin/main"], { cwd: repo.root });
  if (branchTipResult.code !== 0 || mainTipResult.code !== 0) return false;
  if (branchTipResult.stdout.trim() === mainTipResult.stdout.trim()) return false;

  // Path 1: ancestor reachability (fast-forward / classic merge / rebase merges,
  // and the legitimately-stale case of a branch purely behind main).
