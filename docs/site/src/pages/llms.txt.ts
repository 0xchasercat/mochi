...
> One coherent stack for stealth browser automation — relational fingerprint locking, JIT-installed spoofing, behavioral playback, and JA4-impersonating out-of-band HTTP. Bun-native.

mochi is a Bun-native browser automation framework that owns the whole stealth pipeline as one library: pipe-mode CDP transport, a 40-rule relational consistency engine, JIT-friendly inject delivery via Fetch.fulfillRequest, full Bezier+Fitts behavioral synthesis, and a JA4-coherent out-of-band fetch through Bun:FFI to Rust wreq. It runs on stock Chromium-for-Testing — no patched binary, no proprietary infrastructure.

What makes mochi different is the relational thesis: every fingerprint surface — userAgent, platform, WebGL renderer, screen geometry, timezone, fonts, audio, JA4/H2 — derives from one (profile, seed) pair through a deterministic 40-rule DAG. Spoofing is JIT-installed at the Fetch.fulfillRequest seam so the inject reaches the page before any first-party script. Behavioral input is synthesized from Bezier paths + Fitts' law + lognormal-digraph keystrokes. Out-of-band HTTP shares the spoofed Chrome's TLS/H2 fingerprint via wreq. mochi defaults to host-OS matching profiles — on a Linux server, omit \`profile\` and the default is linux-chrome-stable, because Linux is a real-user signal.
...
- [Quickstart](https://mochijs.com/docs/getting-started/quickstart): 5-minute walkthrough — install through first humanClick + JA4 fetch.
- [Install](https://mochijs.com/docs/getting-started/install): Bun + CfT + FFI cdylib setup.
...
- [Consistency engine](https://mochijs.com/docs/concepts/consistency-engine): The 40-rule DAG that derives every fingerprint surface from \`(profile, seed)\`.
...
- [Network FFI](https://mochijs.com/docs/concepts/network-ffi): The Bun:FFI → wreq path; C ABI shape; NetCtx lifecycle.
- [JA4 coherence](https://mochijs.com/docs/concepts/ja4-coherence): Why TLS/H2 must match the spoofed Chrome.
...
- [@mochi.js/net](https://mochijs.com/docs/api/net): TS facade for the FFI.
