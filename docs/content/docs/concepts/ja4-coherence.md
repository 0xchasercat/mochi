...
**2. `Sec-CH-UA*` headers come from the same matrix.** The launch path drives `Network.setUserAgentOverride` with `userAgent` *and* `userAgentMetadata` derived from `MatrixV1.uaCh.*`. Chromium derives every `Sec-CH-UA-*` request header from `userAgentMetadata` — `Sec-CH-UA-Platform`, `Sec-CH-UA-Platform-Version`, `Sec-CH-UA-Arch`, `Sec-CH-UA-Bitness`, `Sec-CH-UA-Mobile`, `Sec-CH-UA-Full-Version-List`. The wreq Client emits the same headers from the same matrix fields. Both surfaces (browser nav + script-side fetch) read the *same* `MatrixV1`, so they cannot drift. PLAN.md task 0261 closed this leak in v0.2.
