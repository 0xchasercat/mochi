...
The Rust crate matches by family (`chrome*` → Chrome handshake), so an unknown major still fingerprints as the right browser — exact-major matching is a phase 0.7 deliverable. See `packages/net-rs/src/ffi/preset.rs` for the canonical list.
