...
Pulls a consolidated visitor record from the harvester API (`MOCHI_HARVESTER_API` env or `--api`, defaults to `http://wrkx.app/api`), normalizes per-category snapshot shape, derives a `ProfileV1`, and emits the canonical 4-file profile dir:
...
The `@mochi.js/cli` package re-exports the browsers helpers so `@mochi.js/core` can resolve the Chromium binary at launch (task 0011).
...
| `MOCHI_HARVESTER_API` | `profiles` | Harvester base URL (default `http://wrkx.app/api`) |
