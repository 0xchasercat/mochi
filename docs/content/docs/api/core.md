...
- `storage()` — snapshot `{ cookies, localStorage: {}, sessionStorage: {} }`. localStorage / sessionStorage are placeholders pending phase 0.7; for live read/write use `Page.localStorage` / `Page.sessionStorage`.
