...
The IDs that ship in the v1 catalog. Pass any of these as `LaunchOptions.profile`. The first batch (`mac-m4-chrome-stable`, `linux-chrome-stable`, `windows-chrome-stable`) was captured natively; the harvester-imported cohort (`mac-chrome-stable`, `mac-chrome-beta`, `windows-chrome-stable`, `mac-brave-stable`) was imported from the harvester corpus per task 0260 and carries a synthesised `wreqPreset` (see PROVENANCE.md).
...
**Phase 0.4 placeholder — currently throws.** Lands in phase 0.4 as the canonical lookup function. For now, if you need to load a profile from disk programmatically:
...
The shape is documented in `tasks/0260-import-harvester-profiles.md` and `PLAN.md §12`. Don't author profiles by hand — capture them with `mochi capture --profile-id <id>` (see [API → mochi CLI](/docs/api/cli)) or import from the harvester with `mochi profiles import`.
...
| Call | Behavior |
| --- | --- |
| `getProfile(id)` | Always throws at v0.0.1 with a "not yet implemented" message; lands in phase 0.4 |
...
Public surface (verbatim from packages/profiles/src/index.ts as of 2026-05-09):
  ...
  getProfile(_id: ProfileId): never                — PLACEHOLDER, throws at v0.0.1, lands in phase 0.4
