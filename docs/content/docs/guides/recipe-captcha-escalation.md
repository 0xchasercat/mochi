...
- **Profile family rotation.** The recipe flips from `mac-m4-chrome-stable` to `windows-chrome-stable` on attempt 2. That's a different `wreqPreset` (`chrome_131_macos` → `chrome_131_windows`), a different UA-CH platform, a different `display.{width, height}`. If the site fingerprinted the macOS shape on attempt 0, the Windows shape is genuinely a different identity.
