...
- **Comparing scores across runs without controlling the seed.** `seed` controls the matrix; same seed = same matrix = same expected score. Different seeds = legitimately different display dimensions, different language ordering, different `wreqPreset` if the profile randomizes it. Pin the seed when comparing.
