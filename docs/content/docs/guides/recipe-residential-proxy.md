...
- **`geoConsistency: "auto-correct"` over a sketchy proxy.** Auto-correct trusts mochi's IP-derived defaults over the user's declared profile. If your proxy provider rotates exit IPs mid-session, the matrix you started with isn't the matrix the page sees later. `"privacy-fallback"` is the safer default for stable identity.
