...
`docs/audits/puppeteer-real-browser.md` notes the `ghost-cursor` integration as MED-impact; mochi's synth is structurally deeper (Fitts + lognormal digraph + profile-keyed parameters) and validated by the conformance suite (task 0150) against CloakBrowser's `superHumanSpeed` / `suspiciousClientSideBehavior` checks.
...
| nodriver | partial — `verify_cf()` uses OpenCV template-matching against a bundled English PNG (`tab.py:1629-1757`). Fragile, locale-dependent, straight-line click. **Negative reference** — the synthesis-of-audits explicitly notes mochi's task 0220 *did not* copy this pattern. |
| undetected-chromedriver | none |
...
Every JS-layer peer defaults to "spoof Windows because browserscan-style surface checks penalize Linux UAs". Browserscan is a surface-level string checker, not a WAF ML model. mochi's design team's position is that the assumption is wrong:
...
mochi closed three of them in v0.2 wave-1 (tasks 0250–0252):
- `MouseEvent.screenX/Y` patch (PRB origin) — task 0250
- `--lang=<matrix.locale>` (udc origin) — task 0251
- `--window-size=<matrix.display.W,H>` (udc origin) — task 0252
...
- [`docs/audits/patchright.md`](https://github.com/0xchasercat/mochi/blob/main/docs/audits/patchright.md) — task 0201
- [`docs/audits/puppeteer-real-browser.md`](https://github.com/0xchasercat/mochi/blob/main/docs/audits/puppeteer-real-browser.md) — task 0200
- [`docs/audits/nodriver.md`](https://github.com/0xchasercat/mochi/blob/main/docs/audits/nodriver.md) — task 0202
- [`docs/audits/undetected-chromedriver.md`](https://github.com/0xchasercat/mochi/blob/main/docs/audits/undetected-chromedriver.md) — task 0203
- [`docs/audits/synthesis.md`](https://github.com/0xchasercat/mochi/blob/main/docs/audits/synthesis.md) — Phase B synthesis: cross-cutting findings, deliberate divergences, out-of-scope items

Each report follows the same structure: **They have / we don't** (cited findings ranked HIGH/MED/LOW), **We have / they don't** (sanity check), **Bench scoring** (if any), and a **Conclusion** that funnels into v0.2 task briefs (`tasks/0250` onward).
