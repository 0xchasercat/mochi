...
- a884318: Fix `TypeError: Illegal invocation` thrown by the `performance-timing` Proxy when page scripts read brand-checked native getters (`responseStart`, `requestStart`, `transferSize`, …) on a `PerformanceNavigationTiming` entry.
